...
SOURCE_CLI = "run_aqa_retry_run_daily_chain_retry_manifest_from_report_rollup.py"
INPUT_ARTIFACT_KIND = "retry_run_daily_chain_report_rollup"
OUTPUT_ARTIFACT_KIND = "retry_run_daily_chain_report_rollup_retry_manifest"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build retry manifest from artists retry-run daily chain report rollup."
    )
...
def main() -> int:
    ...
    if not rollup_path.exists():
        print(f"[ERROR] retry_run_daily_chain_report_rollup_not_found:{rollup_path}")
        return 1

    try:
        rollup_obj = load_json(rollup_path)
    except Exception as exc:  # noqa: BLE001
        print(f"[ERROR] retry_run_daily_chain_report_rollup_load_failed:{rollup_path}:{exc}")
        return 1
    ...
    if not isinstance(raw_failed_runs, list):
        print("[ERROR] retry_run_daily_chain_report_rollup_failed_runs_not_list")
        return 1
    ...
    print(
        "[DONE] retry_manifest_from_daily_chain_report_rollup_generated "
