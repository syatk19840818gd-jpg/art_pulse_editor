...
## 24. 2026-04-22 履歴配置整理（02から04への役割整理）
- 個別 block の完了履歴・数値結果・scope 固有の同期記録は 04 を正とする。
- 02 には詳細索引を残し、block 完了同期の具体履歴は 04 へ集約する。
- `current scope 10館 block 完了同期（2026-04-14 JST）` の個別結果は、本04の履歴記録を参照する。

# 復元本文: 2026-03-25 `10f51bd3` 事故前最新ログ
...
- 判定:
  - Arcadia Missa の重複画像2件について、URL正規化不足を確認。
- ??:
  - `image_url_hash` が raw URL 基準で、`-1024x683` と `-2048x1365` を別画像として扱っていた。
  - 旧 hash metadata fallback が重複再注入を許していた。
- ??:
  - `run_phase1_seed10_artist_image_collect.py`
    - `normalize_image_url_for_dedupe` で `-WxH` suffix を除去。
    - `image_url_hash` を正規化URL基準に統一。
    - metadata merge も normalized URL identity で dedupe。
- ??:
  - 退避1: `_trash/task_a3b_dupfix_arcadia_reset_20260227_234325/`（moved=5）
  - 退避2: `_trash/task_a3b_dupfix_arcadia_rerun_reset_20260227_234524/`（moved=4）
...
- ??:
  - selected URL は4件すべて unique。重複0件。
...
- SSOT参照: 01(4-0,4-4,5-4,6-2,6-3,10) / 02(CARD 10,11,14,16) / 03 / 04
- ??:
  ...
  - run_phase1_seed10_artist_image_collect.py: works404 fallback追加 / dedupe / index連携を確認
- preflight:
  - python run_phase1_network_preflight.py -> exit 0（2回PASS）
- 実行:
...
- ??:
  - The Approach は0/5のまま。
  - Adams and Ollman は skip registry により auto-skip。
...
- 実装修正:
  - `run_phase1_seed10_artist_image_collect.py`
    - orphan画像を `_trash/orphan_artist_images_<ts>/...` へ退避する cleaner を追加。
    - works 404 時の detail fallback と lenient artist match を個別 if なしで追加。
    - seed_supply_by_gallery / seed_supply_under_cap を summary へ出力。
    - 既存 metadata 由来の local_path 欠落を補正。
  - `run_phase1_seed10_artist_image_collect_report.py`
    - seed不足/供給capを report/breakdown に表示。
- preflight:
  - `python run_phase1_network_preflight.py` x2 -> PASS
- 再実測:
  - The Approach
  ...
  - report 3件 + `python run_compare_phase1_guard.py --target-year 2025` -> guard_passed=true
- ??:
  - Tom Allen は payload不足3件を orphan cleanup 後に回復確認。
  - Arcadia は供給seed不足が残り、3/5 で under_cap と判定。
...
## 151. TASK T-108-ARTISTS-TEXT-CLOSE-2（Arcadia/Adams再開対応）
- 実行前確認:
  - 01/02/03/04 確認OK
  - preflight 2回PASS（`phase1_network_preflight_summary_20260301T042809Z.json`）
- 実装修正:
  - `phase1_artist_link_utils.py` の text/image 共通リンク処理を確認。
  - `python -m py_compile phase1_artist_link_utils.py run_phase1_seed10.py run_phase1_seed10_artist_image_collect.py` PASS
- Arcadia/Adams 確認:
  - candidates: Arcadia=18, Adams=20。ともに `>=5` を確認。
  - `data/gallery_lists/skipped_galleries_registry.csv` の2件を解除（backup: `...bak_20260301_132907`）。
- ??:
  - 1回目 `run_phase1_seed10.py --include-artists-text --max-artists-per-gallery 80` は `KNOWN_FAILED_URL_NON_RETRYABLE` で対象が残った。
  - `failed_fetches_artists_seed10_2025.json` の Arcadia/Adams 2件を退避（backup: `...bak_20260301_134210`）。
  - 2回目の再実行で `artists_records_saved_total=42`、Arcadia=18、Adams=20 を確認。
- guard:
...
- R2:
  - `run_phase1_seed10_r2_sync.py --scope raw --dry-run --prune` を確認。
  - guarded apply は guard fail 回避のため未実行。`--prune --require-dry-run-log --max-prune 600` は後続確認対象。
- ??:
  - Arcadia/Adams status = reopened候補: candidates>=5 + 再抽出対象あり。
  - 70%到達判定は後続タスクで継続確認。
...
# 2026-04-22 01〜04 docs復旧ログ（削除事故前提）

- 復旧対象: `docs/01_PROJECT_SPEC_CURRENT_FULL.docx`, `docs/02_RAG_SPEC_DERIVED.md`, `docs/03_STATE_SNAPSHOT_NEXT_TASKS.md`, `docs/04_TASK_PROGRESS_LOG.md`。
- 診断結果: 2026-03-30 `e6de6b2`（`⑤完全削除`）で 01 は 92,154字→835字、02 は約40,511字→1,969字、03 は499,263字→1,634字、04 は346,549字→898字へ急減。削除事故疑いとして採用禁止。
- 採用基底: 2026-02-23 ZIP を安全基底として確認し、git 上の安全増分を 2026-03-25 `10f51bd3` まで回収。
- 事故前最新として `10f51bd3` を 01〜04 の復元基底に採用。
- 2026-03-30 以降の有効追補は、現行 worktree 02/03/04 から救出し、01末尾・02末尾・03参考メモ・04末尾へ追補。
- 2026-03-20〜2026-03-25 の 01 追補に残っていた疑問符連続行は、2026-04-22 の局所修復で該当段落のみ置換した。
- 今回は docs 復旧のみ。実装変更、API実行、R2実行、xlsx更新、RAG抽出、next block 開始、artists image collect verify-first は未実施。

## 再発防止固定

- 01 は追加・局所変更のみ。明示承認なしの全置換・全削除・大規模要約圧縮を禁止する。
- docs 更新は対象段落アンカー指定の局所 patch に限定する。全文再生成を禁止する。
- 更新前後に `git diff --word-diff -- docs/...` を確認し、大量削除、文字化け、章見出し数/主要段落数/文字数の急減があれば中止する。
- Codex task 作成時は `01全置換禁止`、`docs更新は局所patch`、`不自然な削除diffが出たら即中止` を毎回明記する。
- 大きい docs 更新は複数の局所 patch に分割し、文字数/段落数が大幅減少した場合は成功扱いにしない。

---

# 削除事故後ソースから救出した 2026-03-30〜2026-04-22 ログ

以下は現行 worktree 04 から救出したログである。2026-03-30 の大量削除差分を正本本文として採用するものではなく、事故後に入った有効な時系列記録の救出として保持する。
...
- `run_block_closeout.py` に `--exhibitions-raw-trial-root` 導線を追加し、既存の bounded merge 契約に沿って Exhibition raw も 同一scope で narrow 反映できるようにした（gallery/host 固定分岐なし）。
- 同一scope narrow apply（`TASK_PHASE3_EXHIB_TEXT_NARROW_APPLY_20260411T2215JST`）を実行し、`current exhibitions raw` を `59 -> 69` へ更新、workbook 更新を完了した。
...
- workbook 人間確認 OK 後、同一scope narrow config のみで R2 live apply を実行した（`TASK_PHASE3_EXHIB_TEXT_NARROW_SCOPE_R2_APPLY_20260411T2258JST`）。pre-plan は upload 2 / prune 0、apply は uploaded 2 / deleted 0 / failure 0。
- R2 post-check（`TASK_PHASE3_EXHIB_TEXT_NARROW_SCOPE_R2_POSTCHECK_20260411T2300JST`）で `would_upload=0 / would_prune=0` を確認し、同一scope 反映をクローズした。
...
## 25. 2026-04-21 文書同期（リポジトリ整理 / ②整理 / 依存整理 / 作品検索修復クローズ）
- 本節は「前回文書更新（2026-04-18）以降」に本チャットで実施・受理された事項のみを追記する。
- まずリポジトリ整理を実施し、不要な試行用・一時・旧補助ファイルを削除した。`data/history/**` は保持判定、safe D のみ削除、protected 系欠損なしを確認した。
- 続いて②整理を完了し、`app.py` の検索UI重複整理と不要セッションキー削除、`run_enrichment_seed10.py` の正規ビルダ寄せ、`run_compare_phase1_guard.py` の不要取り込み削除、`run_closeout_exhibitions_from_current.py` の引退ランナー化を受理した。
- 現行専用画像強化を受理し、機能1/4/7の画像系は現行専用前提を維持した。画像表示確認は成立。
- 依存整理として `requirements.txt` に `openpyxl` / `numpy` を追加、`runtime.txt` に `python-3.11.9` を追加、`requirements.lock` を導入し、README の通常導入導線を `pip install -r requirements.lock` 前提へ最小更新した。
- 依存運用方針は `requirements.txt` を一次定義、`requirements.lock` を再現用、`runtime.txt` を Python 基準として固定した。
- 作品検索の画像なし再発は先行検証で主因を「ヒット選定非抑制 + 旧パス / メタデータ / `id_map` 不整合」と確定した。
- 修復案としてプレビュー優先ロジックを採用した。
- `payload_hash` 全面結合補正は一度試行したが回帰を起こしたため撤回した（採用しない）。
- `app.py` の「警告（ArtWork Search）」表示は削除した。
- `SAFE_FULL 117` の限定補正を導入し、`UNRESOLVED 150` は `dry-run` で旧ローカル実体全件存在を確認後、限定昇格で現行キャッシュ側救済を実施した。
- 全4636行の再監査でプレビュー表示可能 `4636/4636`、画像なし残件 `0` を確認した。
- `liste` metadata 側未ベクトル化 40 件は限定ベクトル化 + 限定追記を実施し、ベクトル成果物行数は `4636 -> 4676` へ更新した。
- ユーザー確認として「全画像が出ない回帰なし」「件数・並び・速度に大きな違和感なし」を受理した。`liste` 側多様性増加の体感は未確認だが追記成功を受理した。
- 本チャット根拠では R2 sync の実行有無は未確認のため、同期済み扱いは記載しない。

## 26. 2026-04-21 文書固定更新（`rag_gellery_breakdown_master.xlsx` 早期確認追加 + 17相当維持 + 2/23→3/23 成果物受け渡し再発防止）
- 本節は文書更新のみで実施し、実装変更・API実行・currentデータ変更・R2実行・`rag_gellery_breakdown_master.xlsx` 実更新・次 block 開始は行っていない。
- 01を正本として、Phase 3 block 必須工程を `1〜24` に再固定した。
- `rag_gellery_breakdown_master.xlsx` 人間確認は2段階で固定した。工程5に「抽出率・館ごとの中身確認の早期確認」を追加し、closeout 後の後段確認（工程18、従来17相当の役割）は維持した。
- closeout 後の順序は維持し、`run_block_closeout apply（R2なし）` 後に後段 `rag_gellery_breakdown_master.xlsx` 人間確認を行い、その後 `current_required_rag_full` の `plan / apply / post-check` へ進む契約を継続した。
- 2/23→3/23 の成果物受け渡し条件を再発防止として固定した。`2/23 dry-run 診断` は診断JSONのみで 3/23 用 artist raw成果物を生成しないことを明記した。
- 3/23 成立条件として、2/23 を同一scope の rebuild verify-first で実行し、`--include-artists-text` 付きで同一 `trial_root` / 同一 `run-id` 配下に collector 入力 raw（`data/current/raw/artists_*_2025.jsonl`）を作る契約を明記した。
- 禁止事項として `2/23 dry-run 診断 -> そのまま 3/23` を明文化し、gallery個別回避ではなく運用導線契約で再発防止する方針を固定した。
- 02には圧縮索引（工程一覧・closeout前ゲート）として反映し、03には現在地運用注意として短く同期した。

## 27. 2026-04-21 文書固定修正（工程5/18の確認対象を `rag_gellery_breakdown_master.xlsx` に明示固定）
- 本節は文書更新のみで実施し、実装変更・API実行・currentデータ変更・R2実行・`rag_gellery_breakdown_master.xlsx` 実更新・次 block 開始は行っていない。
- 工程5は「`rag_gellery_breakdown_master.xlsx` を更新し、抽出率・館ごとの中身を人間確認する早期確認」として明示固定した。
- 工程18は「closeout / cleanup 反映後に更新済み `rag_gellery_breakdown_master.xlsx` を人間確認する最終整合確認」として明示固定した。
- `_trial` 配下の CSV / MD / JSON は確認補助資料であり、`rag_gellery_breakdown_master.xlsx` の代替ではないことを明記した。
- 工程5 / 工程18 は `rag_gellery_breakdown_master.xlsx` を確認して初めて完了扱いとする運用へ固定した。

## 28. 2026-04-21 文書固定修正（current raw と `rag_gellery_breakdown_master.xlsx` の対応保証ルール）
- 本節は docs-only で実施し、実装変更・API実行・currentデータ変更・R2実行・`rag_gellery_breakdown_master.xlsx` 実更新・次 block 開始は行っていない。
- `rag_gellery_breakdown_master.xlsx` の館行は current formal artifacts 起点で更新されること、かつ gallery 単位の mixed 集計（artists / exhibitions 同居表示）であることを 01/02/03 に固定した。
- 片側カテゴリ 0/0（例: artists 側または exhibitions 側）が存在しても、もう片側カテゴリの current raw が存在しうる運用を明文化した。
- scope 対象館は、該当カテゴリ 0件でも館行として更新対象に残りうることを明記した。
- 人間確認時の追跡根拠を `data/current/raw/*.jsonl`、`data/current/images/metadata/*.jsonl`、closeout report、`rag_gellery_breakdown_master.xlsx` のメモ列 / 更新日 / run_id に固定した。
- 「`rag_gellery_breakdown_master.xlsx` に館行があるのに current raw が無いように見える」場合は、館一括ではなく artists / exhibitions を分離して突合すること、説明不能な館行を合格扱いにしないことを再発防止ルールとして固定した。

## 29. 2026-04-21 文書固定修正（artists enrichment preflight の io_root整合ルール）
- 本節は docs-only で実施し、実装変更・API実行・currentデータ変更・R2実行・`rag_gellery_breakdown_master.xlsx` 実更新・次 block 開始は行っていない。
- 新blockの 6/24（artists enrichment preflight）前に、scope 対象 request 母集団の存在を同一scope / 同一io_root で確認する運用を 01/02/03 に固定した。
- 6/24 は scope・io_root・request lane が整合した状態で実行し、`exit 0` / `status ok` でも `scoped_request_rows=0` または `target_rows=0` は blocker 扱いとする。
- `target_rows=0` の場合は 7/24（artists enrichment submit）へ進まないことを明文化した。
- blocker時は、同一Task内で `request母集団確認 -> 必要なら same scope / same io_root で request 生成 -> preflight再実行` まで行い、不要なTask分割をしない運用に固定した。

## 30. 2026-04-22 実装修正（`rag_gellery_breakdown_master.xlsx` Artist側 0 反映の再発防止）
- `rag_gellery_breakdown_master.xlsx` の新ブロック行で Artist側 B〜H が 0 になった原因は、closeout apply が artists enrichment / artists text vector / artist works images vector は trial root から反映した一方で、Artist側 B〜H の根拠になる artist raw と artist image metadata を同じ trial/current 文脈で current_write に反映していなかったことだった。
- `run_block_closeout.py` に generic な正式引数として `--artists-raw-trial-root` と `--artists-image-metadata-trial-root` を追加し、gallery 固定分岐を増やさずに same scope の artist raw / artist image metadata を current_write へ統合できるようにした。
- 誤った Artist側 0 を根拠に前回 auto skip へ入った `Galerie Karin Guenther` と `Nicoletti` は、same scope の再反映前に skip registry から除外した。`Hauser & Wirth` は人間確認済み skip のまま維持した。
- same scope 9館のみを対象に `rag_gellery_breakdown_master.xlsx` を再反映し、Grimm / Hales / Galerie Max Hetzler / Magenta Plains などで Artist側 B〜H が 0 ではない状態へ戻した。Exhibition側 I〜M と合計数表示は維持した。
- 再発防止として、closeout apply / xlsx_update では artists / exhibitions それぞれの raw と image metadata の根拠入力群が同一scope / 同一trial-current文脈で解決されていること、apply 後に Artist側 B〜H / Exhibition側 I〜M の片側だけが説明不能に 0 になっていないことを確認する運用を 01/02/03 に固定した。
- R2実行・次block開始は行っていない。

## 31. 2026-04-22 実装修正（enrichment 適用出力の正規監査出力化）
- `data/current/enrichment/*_enrichment_apply_output*.jsonl` を `current` 採用済み enrichment 結果の正規監査出力として整理した。成功した `current` 採用 1 元レコード = 1 JSONL 行とし、同一 `fair_slug + text_hash + source_url` の二重追加を禁止した。
- artists 側のズレは `current` 側の適用出力に追記重複が残っていたことが原因だった。`artists_enrichment_apply_output.jsonl` は `1720 -> 1459` に正規化し、重複 261 件を除外した。
- exhibitions 側のズレは `current` 生データには 609 件の enrichment 結果が採用済みだった一方、`current` 側の適用出力が 500 件のままだったことが原因だった。`exhibitions_enrichment_apply_output_2025.jsonl` は `500 -> 609` に正規化した。
- `run_block_closeout.py` は `--exhibitions-enrichment-trial-root` を受け取り、artists / exhibitions とも trial enrichment を `current` に採用する際に同じ正規化ルールを通す導線へ揃えた。
- `enrichment_batch_common.py` の materialized current builder は current/history の APPLIED duplicate key 数を summary に出すようにし、再実行時の重複混入を監査しやすくした。
- `rag_gellery_breakdown_master.xlsx` との照合はカテゴリ別に行う。`rag_gellery_breakdown_master.xlsx` は raw テキスト行数、適用出力は `current` 採用済み元レコード件数なので、artists の差分 12 件は raw 重複 6 件 + 未適用元レコード 6 件として扱い、exhibitions は 609 / 609 で一致した。
- API / R2 / 次 block は実行していない。

## 32. 2026-04-22 文書同期（Phase 3 今回 block 完了内容）
- 本節は docs-only で実施し、実装変更・API実行・currentデータ変更・R2実行・`rag_gellery_breakdown_master.xlsx` 実更新・次 block 開始は行っていない。
- 今回 block は 2/24 raw verify-first、3/24 artists image collect verify-first、4/24 exhibitions image collect verify-first、5/24 `rag_gellery_breakdown_master.xlsx` 人間確認を完了した。
- 6/24〜9/24 artists enrichment preflight / submit / check / apply、10/24〜13/24 exhibitions enrichment preflight / submit / check / apply を完了した。
- 14/24 artists text vector verify-first、15/24 artist works images vector verify-first、16/24 run_block_closeout verify-first、17/24 run_block_closeout apply（R2なし）、18/24 `rag_gellery_breakdown_master.xlsx` 再確認を完了した。
- 19/24 `current_required_rag_full plan`、20/24 `current_required_rag_full apply`、21/24 `current_required_rag_full post-check` を完了した。post-check は `would_upload=0` / `would_prune=0`。
- `Hauser & Wirth` は人間確認済み skip のまま維持し、scope外の館を更新対象へ戻さない。
- closeout で必要だった正式引数 `--artists-raw-trial-root`、`--artists-image-metadata-trial-root`、`--exhibitions-image-metadata-trial-root` を 01 / 02 / 03 / 04 に同期した。
- enrichment 適用出力の正規監査出力ルール、artists 1459 件 / exhibitions 609 件、artists の 12 件差分理由（raw 重複 6 件 + 未適用元レコード 6 件）を 01 / 02 / 03 / 04 に同期した。
- 現在地は今回 block 完了、次 block 再開判定待ち。次にやることは未定で、人間指示待ちとする。

## 33. 2026-04-22 現行 docs 局所修復（文字化け / NUL 除去）
- 本節は docs-only の復旧ログであり、実装変更・API実行・R2実行・xlsx更新・次 block 開始は行っていない。
- 現行 01〜04 を最新版として直接参照し、疑問符連続行・単独疑問符化け・置換文字・NUL 混入などの壊れ候補だけを機械検出した。
- 01 は疑問符連続と単独疑問符化けの壊れ段落のみを局所置換し、周辺本文・章順・既存構成は変更していない。
- 04 は NUL / 置換文字が混入した `TASK T-116-EXHIBITIONS-IMAGE-MAX7` ブロックを、git 履歴の同一ログ本文から局所復元した。
- 03 は現在地把握に支障が出る後半の疑問符連続行のみを局所修復した。
...
## 36. 2026-05-04 RAG?????????21k?23h?

- 21k: current RAG integrity gate GO?runtime `_trial` refs ? 0 ????
- 21l: `run_block_closeout.py --current-only` ? image cache promotion ????????
  - commit: `a982a34`
- 21m?21o: `current_required_rag_full` ? plan/apply/post-check ??????post-check all zero????
- 21p: app/RAG?????????smoke???? GO?
- 23a?23h: `_trial` cleanup ? verify-first ? apply ??????
  - root cleanup
  - remaining cleanup
  - protected reassessment cleanup
  - loose files cleanup
- 23g-fix: Art Work Search ? `_trial` loose file??????
  - commit: `efdaa92`
- 23h????? `_trial` ? 0 bytes / files 0 / folders 0?
...
- ????????????
  ...
  - 23. _trial cleanup verify-first / apply
  - 24. docs??
- 4???current integrity gate GO / R2 post-check all zero / app smoke GO / runtime `_trial` ????????????????? `_trial` ???cleanup??????
- `_trial` ??????????????????????????
