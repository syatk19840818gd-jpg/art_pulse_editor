...
LAST_UPDATED: 2026-02-23 21:58 JST
...
- Phase 1：RAG抽出パイプライン成立（seed10で安定稼働させる）
  - 直近の到達目標：数値ウォッチ項目の設定外出し（TASK 17）を成立させる
...
[ ] 17) 数値ウォッチ項目を設定ファイル化する（compare guardの運用固定）
    - 目的：`700/180` などのウォッチ数値をコード直書きから外し、運用で更新可能にする
    - 制約：Tarutani_Text は回答根拠セクションとしてのみ使用し、検索結果一覧へは出さない
    - 成立条件：
      - compare CLI が設定ファイル（例: `config/tarutani_answer_guard_profile.json`）を読める
      - 未指定時は既定設定で後方互換動作する
      - summary に使用した guard profile 情報が保存される
      - 03のCHANGELOGに反映される
...
------------------------------------------------------------
TASK 17) 数値ウォッチ項目を設定ファイル化する（compare guardの運用固定）
------------------------------------------------------------
目的：
- 比較ガードで使う主要数値（例: 700/180）を設定ファイルで管理し、運用変更をコード編集なしで行えるようにする。

参照ファイル：
- 01（SSOT）機能⑤（Tarutani_Textの使い方）
- 02（索引）CARD_ID: 16_TARUTANI_TEXT_SCOPE
- run_compare_tarutani_answers.py
- config/tarutani_text_search_profile.json
...
完了条件：
- compare CLI が guard profile（例: `config/tarutani_answer_guard_profile.json`）を読める
- `--watch-numbers` 未指定時は profile 優先、profile未配置時は既定値で後方互換動作する
- summary に `guard_profile_path` / `watch_numbers_effective` が保存される
