...
LAST_UPDATED: 2026-03-01 00:42 JST
...
- Phase 1：RAG抽出パイプライン成立（seed10で安定稼働させる）
  - 直近の到達目標：TASK108で 10ギャラリーのExhibitions画像抽出を最小スコープで開始し、段階拡張の基準を確定する
...
[x] 107) 10ギャラリーのArtistsテキスト抽出を実行し、共通スキップ適用 + 共通R2同期適用を確認してExhibitions抽出フェーズへ進む（本体前進）
    ...
    - 実測値：`artists_records_saved_total=0` / `artists_existing_records_total=225` / `artists_skipped_total=8`（既存重複スキップのみ）
...
[ ] 108) 10ギャラリーのExhibitions画像抽出を最小スコープ（初回は各gallery 1 artist）で実行し、共通スキップ + 共通R2同期を確認する（本体前進）
...
- 例:
  - python run_phase1_seed10_r2_sync.py --scope raw --dry-run --prune
  - python run_phase1_seed10_r2_sync.py --scope raw --prune --require-dry-run-log --max-prune 600
...
------------------------------------------------------------
TASK 108) 10ギャラリー Exhibitions画像抽出（最小開始 + 段階拡張）
------------------------------------------------------------
運用ルールは前回と同じ。参照は 01/02/03/04 のみ。 

RECOMMENDED_MODE: high

TASK T-108-EXHIBITIONS-IMAGES) 10ギャラリーのExhibitions画像抽出を最小スコープで開始し、共通スキップ運用と共通R2同期運用を確認する

【必須参照（章ID明示）】
- 01（SSOT）
  - 4-0) ①～④共通ルール
  - 4-0-A) 既知URL/既知画像の“記憶”と自動スキップ（必須）
  - 4-3) Exhibitions：画像（抽出ルール）
  - 5-2) Exhibitions Images（4-3の成果物）
  - 5-8) 同期方式（R2正本 + local cache）
  - 6-2) ドメイン専用ハードコード増殖の禁止
  - 6-3) 品質ライン（試作10ギャラリーの運用）
  - 10) 仕様変更ルール（事故防止）
...
3) 最小スコープで実行
- 初回は各gallery 1 artist 相当の最小対象で実行する
- 成功済み画像は削除しない（再抽出しない）
...
- 実装:
  - `run_phase1_seed10_r2_sync.py`
    - `--require-dry-run-log` 追加（同一scopeの直前dry-run必須）
    - `--max-prune` 追加（削除候補上限、超過時GUARD_BLOCKED）
  - `run_tarutani_r2_sync.py`
    - `--require-dry-run-log` 追加
    - `--max-prune` 追加
    - 既定scopeを `all` に変更（source偏重の再発防止）
- 運用テンプレ（固定）:
  - 1コマンド運用（推奨）:
    - `python run_r2_sync_runbook.py`
  - Phase1:
    - `python run_phase1_seed10_r2_sync.py --scope all --dry-run --prune`
    - `python run_phase1_seed10_r2_sync.py --scope all --prune --require-dry-run-log --max-prune 600`
  - Tarutani:
    - `python run_tarutani_r2_sync.py --scope all --dry-run --prune --prune-prefix tarutani/source`
    - `python run_tarutani_r2_sync.py --scope all --prune --prune-prefix tarutani/source --require-dry-run-log --max-prune 100`
- 基準スナップショット（現行正）:
