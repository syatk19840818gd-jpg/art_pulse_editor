...
- 現在地は `01〜04 docs復旧中`。この状態を最新の現在地として扱う。
- 今回タスクでは、実装変更、RAG抽出、API実行、R2実行、xlsx更新、next block 開始を行わない。
- `artists image collect verify-first` には進まない。復旧完了後も、次 block 再開は人間の明示指示待ちとする。
- 復旧の採用方針は `2026-02-23 ZIP基底 + 2026-02-23〜2026-03-25安全git差分 + 2026-03-30以降の有効追補救出` とする。
- 2026-03-30 `e6de6b2` の大量削除・圧縮差分は正本採用しない。

## 次タスク

- [x] 01〜04 の復旧診断を行う。
- [x] 01〜04 を事故前安全到達点から復元し、削除事故後の有効追補を救出する。
- [x] docs 更新再発防止ルールを 01/02/03/04 に固定する。
- [ ] 人間確認: 復旧後 diff と 01 本文の保持状況を確認する。
- [ ] 人間確認後の明示指示待ち。RAG抽出・次 block は未着手。

## docs 更新ゲート（今後の Codex task 固定）

- 01 は全置換禁止、全削除禁止、大規模要約圧縮禁止。変更対象外の章・段落は触らない。
- docs 更新は局所 patch のみ。対象段落アンカー、追加/変更理由、想定 diff を task に書く。
- 更新前後に `git diff --word-diff -- docs/...` を確認し、不自然な大量削除、文字化け、章見出し数/主要段落数/文字数の急減があれば中止する。
- 文字数・段落数が大幅に減った場合は成功扱いにしない。

---

# 参考履歴（旧本文・復元保存）: 2026-03-25 `10f51bd3` 事故前最新 03

- ここから下は復元保存した旧本文であり、現行の運用判断は本ファイル冒頭の「現在地スナップショット」「次タスク」「docs 更新ゲート」を優先する。
- 旧本文中に残る `x/24` 表記は当時の記録であり、現行の正式順序は 1〜23 とする。
...
- Phase 2 app run-state (current baseline):
  ...
  - Artist current contract sync note (2026-03-25): Feature 3 / 4 / 7 now read Artist current via shared path/resolver APIs (`phase2_art_pulse_config.py` + `phase2_common_readonly.py` + artist/artwork readonly readers).
  - Artist current fixed-name migration sync note (2026-03-25): app-facing Artist current fixed names are yearless (`data/current/enrichment/artists_enrichment_apply_output.jsonl`, `data/current/vector/artists/*`, `data/current/vector/artist_works_images/*`), while raw/cache/retained ledgers remain year-scoped provenance/audit lanes.
  ...
  - verify-first run-state note (2026-03-25): Feature 1 / 2 / 3 / 4 / 7 passed py_compile + import + minimal runtime-smoke without P0 blockers after current naming migration.
  - verify-first candidate note (2026-03-25): Feature 3 Artist Search still reads formal raw (`artists_*_2025.jsonl`) as runtime listing input; this remains an unresolved design candidate and was not changed in this docs sync.
  - verify-first candidate extension note (2026-03-25): raw dependency is not Feature-3-only; it also reaches Feature 1 directly and Feature 4 via the Feature 3 loader path. This is currently non-blocking.
  - tiny-fix note (2026-03-25): no immediate tiny fix is required from the latest verify-first sweep.
  - R2 apply-upload status note (2026-03-25): verify-first was completed for the relevant current scopes, and `apply-upload` is completed for `enrichment_current_primary`, `artists_vector_current`, and `artist_works_images_vector_current`; yearless Artist app-facing current artifacts are reflected in the remote current family.
  - R2 post-apply re-plan note (2026-03-25): the same three scopes converged to `would_upload=0` after apply-upload; old `_2025` remote current artifacts remain as prune candidates and `apply-prune` is intentionally pending as a separate step.
  - R2 unaffected-lane note (2026-03-25): `raw_current_primary`, `images_metadata_current`, `images_cache_current`, and `current_ledgers_family` were not involved in this upload pass (verify-first baseline `would_upload=0 / would_prune=0`), preserving year-scoped provenance/audit lanes.
  ...
  - Feature 7 tiny-fix note (2026-03-24): single-wide image rendering is aligned to Feature 2 contain-style (`background-image` + `background-size: contain` + centered no-repeat) to avoid narrow-window cropping while preserving one-image full-width fixed-height layout.
  ...
  - next-task note (2026-03-25): after this docs sync, keep `apply-prune` as an explicit separate task only if requested; otherwise return to the main roadmap baseline. Keep the raw dependency spread across Feature 1/3/4 as a future verify-first candidate, and do not reopen cleanup/proof lanes without explicit user task.
...
- 2026-03-25 JST: DOCS-ONLY R2 sync execution status update. Reflected that R2 verify-first was completed for current relevant scopes and `apply-upload` was executed for `enrichment_current_primary`, `artists_vector_current`, and `artist_works_images_vector_current`; yearless Artist current artifacts are now reflected in remote current families and post-apply re-plan converged to `would_upload=0` on those scopes. Old `_2025` remote current artifacts remain prune candidates (`apply-prune` pending by design as a separate step). `raw_current_primary` / `images_metadata_current` / `images_cache_current` / `current_ledgers_family` were not involved in the upload pass. No code/data/app/R2/config changes in this docs sync and no rerun/re-extraction/rebuild/promote.
- 2026-03-25 JST: DOCS-ONLY verify-first sync for Feature 1/2/3/4/7 post-migration state. Recorded that current naming migration + shared resolver alignment keeps runtime stable (no P0 blocker) with py_compile/import/minimal smoke passed, tiny fix not required, and next recommended step is docs sync -> R2 verify-first. Added design-note that raw dependency is currently non-blocking but spans Feature 1 (direct), Feature 3 (direct), and Feature 4 (via Feature 3 loader), so future strict app-facing-current hardening should evaluate 1/3/4 together. No code/data/app/R2/config change and no rerun/re-extraction/rebuild/promote.
- 2026-03-25 JST: DOCS-ONLY storage-lane sync (verify-first reflected). Synced 01/03/04 to current Artist storage state: app-facing Artist current fixed names are yearless for enrichment/vector lanes, raw/cache/retained ledgers remain year-scoped provenance/audit lanes, Exhibition remains year-scoped, Artist Text first-write-wins and Artist Works Images same-source yearly diff append-only remain fixed. Added verify-first candidate note that Feature 3 still reads raw (`artists_*_2025.jsonl`) and is not resolved in this docs sync. No code/data/app/R2/config change and no rerun/re-extraction/rebuild/promote.
...
- Next priority:
  - TASK A-5-CLOSE-1（`liste / anca-potera-u-gallery` の logo/icon 混入確認）を次優先とする。
...
- 目的: `MAX_ARTISTS_PER_GALLERY` を 5 へ引き上げ、artist works images を再実測する。
- 結果: processed=43, ge_target=32, success_rate=74.42% (threshold_passed=true)
- 達成館: Arcadia Missa / Athr / Gallery Baton / Addis Fine Art / Afriart Gallery / Anca Poteraşu Gallery
- 未達/要確認: The Approach（0/5 継続）, Amanita（4/5 許容）
- 補足: Adams and Ollman は skip registry により自動スキップ。
- 次: The Approach を5枚達成へ戻すため、2名分の再取得を優先する。


## TASK MAX5-STABILITY-1（2026-02-28）
- 目的: `orphan cleanup` + `works404 fallback` + `seed_supply summary` を個別 if なしで安定化する。
- ??:
  - The Approach: 5名中 ge_target 0 -> 2。Sara/Anderson=5、Tom系は orphan cleanup 後に継続確認。
  - Arcadia Missa: 3名中5枚達成。seed不足由来の 3/5 を under_cap と判定。
  - A+ Works of Art: 5名中 ge_target=1。seed/候補不足を継続課題化。
- 次: The Approach の残3名 Tom/Phillip/Helene を再現性確認し、6-2 に従って汎用修正する。
...
# 削除事故後ソースから救出した現在地メモ（参考・正本現在地ではない）

以下は現行 worktree 03 から救出した有効メモである。上部の `docs復旧中` を現在地の正として優先する。

03_状態スナップショット_次タスク
プロジェクトスラッグ: ART_PULSE_EDITOR
最終更新: 2026-04-22 JST（実装固定更新: `rag_gellery_breakdown_master.xlsx` 反映時の根拠入力群整合ルール）
...
現在地スナップショット
- リポジトリ整理を完了。不要な試行用・一時・旧補助ファイルを削除し、`data/history/**` は保持、safe D のみ削除、protected 系欠損なしを確認した。
- ②整理を完了。`app.py` の検索UI重複整理と不要セッションキー削除、`run_enrichment_seed10.py` の正規ビルダ寄せ、`run_compare_phase1_guard.py` の不要取り込み削除、`run_closeout_exhibitions_from_current.py` の引退ランナー化を反映済み。
- 現行専用画像強化を受理し、機能1/4/7の画像系は現行専用前提を維持。画像表示確認は成立した。
- 作品検索の画像なし再発は修復クローズ。`SAFE_FULL 117` 補正と `UNRESOLVED 150` の限定昇格救済を実施し、再監査でプレビュー表示可能 4636/4636、画像なし残件 0 を確認した。
- `liste` metadata 側未ベクトル化 40 件は限定ベクトル化 + 限定追記を実施し、ベクトル成果物行数は 4636 から 4676 へ更新された。
- ユーザー確認として、全画像回帰なし・件数/並び/速度に大きな違和感なしを確認。`liste` 多様性増加の体感は未確認だが追記成功は受理済み。
- Phase 3 block 必須工程は、工程5/工程18ともに `rag_gellery_breakdown_master.xlsx` を確認対象とする2段階確認を固定した。
- 3/23→4/23 は同一scope / 同一trial_root / 同一run-id の artist raw成果物受け渡しを必須とし、`3/23 dry-run 診断 -> そのまま 4/23` を禁止する固定ルールを同期済み。

現行運用の要点
- 現行仕様には最終採用状態のみを反映し、試行錯誤は進捗ログ（04）で管理する。
- 依存運用方針は `requirements.txt` を一次定義、`requirements.lock` を再現用、`runtime.txt` を Python 基準とする。
- 本チャット根拠では R2 sync 実行有無は未確認のため、同期済み前提は置かない。
- `rag_gellery_breakdown_master.xlsx` 契約は2段階確認を維持する。工程5は抽出率・館ごとの中身確認の早期確認、工程18は closeout / cleanup 反映後の最終整合確認とする。
- `_trial` 配下の補助CSV/MD/JSONは確認補助資料としてのみ扱い、`rag_gellery_breakdown_master.xlsx` の代替で完了扱いにしない。
- 4/23 実行前提は、3/23 側で `--include-artists-text` 付き rebuild verify-first を同一 `trial_root` / 同一 `run-id` で実行し、collector 入力 raw成果物を同一trialへ作成済みであることとする。
- `rag_gellery_breakdown_master.xlsx` の館行は mixed 集計（artists / exhibitions 同居表示）であるため、必要時の current raw 突合はカテゴリ別に行う。
- artists 側または exhibitions 側の片側 0/0 を、館全体 raw なしと誤読しない。反映根拠は current raw / image metadata / closeout report / メモ列 / 更新日 / run_id で追跡する。
- 工程6（artists enrichment preflight）前に、scope 対象 request 母集団の存在（同一scope / 同一io_root）を確認する。
- closeout apply / xlsx_update 前に、Artist側は artist raw / artist image metadata、Exhibition側は exhibition raw / exhibition image metadata の根拠入力群が同一scope / 同一trial-current文脈で current_write に入っていることを確認する。
- closeout apply 後の `rag_gellery_breakdown_master.xlsx` では、Artist側 B〜H と Exhibition側 I〜M を分けて確認し、片側だけ説明不能に 0 になっていないかを見る。説明不能な片側0は完了扱いにしない。
- 工程7で `exit 0` / `status ok` でも `scoped_request_rows=0` または `target_rows=0` は blocker 扱いとし、8/23へ進めない。

次タスク
- [x] このチャットで確定した 01〜04 文書同期を完了する。
- [x] リポジトリ整理・②整理・作品検索修復クローズを文書へ反映する。
- [ ] 次の人間指示待ち（新規実行は未着手）。
...
2026-04-22 現在地（Phase 3 今回 block 完了）
- 今回 block は 3/23 raw verify-first から 22/23 `current_required_rag_full post-check` まで完了した。
- `rag_gellery_breakdown_master.xlsx` は再反映後の人間確認 OK。`Hauser & Wirth` は人間確認済み skip のまま維持する。
- closeout で必要だった正式引数は `--artists-raw-trial-root`、`--artists-image-metadata-trial-root`、`--exhibitions-image-metadata-trial-root` として同期済み。
...
- current integrity gate: GO
- R2 post-check: all zero?would_upload=0 / would_prune=0 / missing=0 / remote_only=0 / size_mismatch=0?
- app smoke: GO
- runtime `_trial` ??: ??
- `_trial`: 0 bytes / files 0 / folders 0??????????????

### CHANGELOG????

- image cache??? current ????
- metadata / id_map ? `_trial` path ? current path ?????
- `current_required_rag_full` ??plan??apply?post-check all zero?????
- `run_block_closeout.py --current-only` ? image cache promotion ??????
  - `a982a34 fix current-only closeout image cache promotion`
- Art Work Search ? `_trial` loose file runtime??????
  - `efdaa92 remove artwork search trial loose file dependency`
- `_trial` cleanup?root / protected / loose files?????

### ???????

- ????? `20 ? 21 ? 22 ? 23 ? 24` ????
- 4???integrity GO / post-check all zero / app smoke GO / runtime `_trial` ????????? `_trial` ???cleanup???
- cleanup ??? verify-first ? apply?
