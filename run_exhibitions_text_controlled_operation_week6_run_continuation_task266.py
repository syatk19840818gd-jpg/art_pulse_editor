...
READY = "CONTROLLED_OPERATION_WEEK6_SUCCESS"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK266 Exhibitions Text controlled operation week6 run and holding recheck scheduling"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument(
        "--task265-ready-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_input_task265.csv",
    )
    parser.add_argument(
        "--task265-escalate-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_input_task265.csv",
    )
    parser.add_argument(
        "--task265-ready-summary-json",
    ...
    parser.add_argument("--expected-prior-run-id", default="task265_20260305T080334Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task266_%Y%m%dT%H%M%SZ")
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    state_path = Path(args.state_latest_json)
    ready_rows = read_csv(Path(args.task265_ready_input_csv))
    escalate_rows = read_csv(Path(args.task265_escalate_input_csv))
    summary265_ready = read_json(Path(args.task265_ready_summary_json), default={})
    summary265_escalate = read_json(Path(args.task265_escalate_summary_json), default={})
    ...
    integrity_ready = dict(summary265_ready.get("common_checks") or {})
    integrity_escalate = dict(summary265_escalate.get("common_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task266_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "controlled_operation_week6_run_and_holding_recheck_scheduling",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK266_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_summary",
        "task": "TASK266",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task265_ready_input_csv": str(Path(args.task265_ready_input_csv)),
            "task265_ready_summary_json": str(Path(args.task265_ready_summary_json)),
        ...
        "next_task_recommendation": {
            "id": "TASK267",
            "title": "EXHIBITIONS-TEXT-HOLDING-RECHECK-PREPARATION-AFTER-TASK266",
            "ja": "Prepare controlled holding-lane recheck flow after task266 trigger checkpoint",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_summary",
        "task": "TASK266",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task265_escalate_input_csv": str(Path(args.task265_escalate_input_csv)),
            "task265_escalate_summary_json": str(Path(args.task265_escalate_summary_json)),
            "state_latest_json": str(state_path),
        },
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_input_task266.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_summary_task266.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_manifest_task266.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_input_task266.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_summary_task266.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_manifest_task266.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_monitored_diff_task266.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_preflight_task266.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_backup_log_task266.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_by_fair_task266.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_by_gallery_task266.csv"
    )
    report_path = output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_task266.md"
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_manifest",
        "task": "TASK266",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_manifest",
        "task": "TASK266",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK266 Exhibitions Text controlled operation week6 run and holding recheck scheduling",
        "",
        "## run",
