...
READY = "CONTROLLED_OPERATION_STARTED_SUCCESSFULLY"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK261 Exhibitions Text controlled operation start weekly sync dual lane"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument("--expected-prior-run-id", default="task257_20260305T051315Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task261_%Y%m%dT%H%M%SZ")
    ...
    summary257_ready = read_json(Path(args.task257_ready_summary_json), default={})
    summary257_escalate = read_json(Path(args.task257_escalate_summary_json), default={})
    ...
    integrity_ready = dict(summary257_ready.get("common_checks") or {})
    integrity_escalate = dict(summary257_escalate.get("common_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task261_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "controlled_operation_start_weekly_sync_dual_lane",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK261_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_ready_summary",
        "task": "TASK261",
        "run_id": run_id,
    ...
    escalate_summary = {
        "artifact": "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_escalate_summary",
        "task": "TASK261",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_ready_input_task261.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_ready_summary_task261.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_ready_manifest_task261.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_escalate_input_task261.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_escalate_summary_task261.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_escalate_manifest_task261.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_monitored_diff_task261.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_preflight_task261.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_backup_log_task261.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_by_fair_task261.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_by_gallery_task261.csv"
    )
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_ready_manifest",
        "task": "TASK261",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_controlled_operation_start_weekly_sync_dual_lane_escalate_manifest",
        "task": "TASK261",
        "run_id": run_id,
