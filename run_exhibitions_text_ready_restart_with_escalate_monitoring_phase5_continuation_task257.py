...
READY = "READY_FOR_STEADY_STATE_OPERATION_PROPOSAL"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK257 Exhibitions Text ready restart with escalate monitoring phase5 continuation"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument(
        "--task256-ready-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_ready_restart_with_escalate_monitoring_phase4_continuation_ready_input_task256.csv",
    )
    parser.add_argument(
        "--task256-escalate-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_ready_restart_with_escalate_monitoring_phase4_continuation_escalate_input_task256.csv",
    )
    parser.add_argument(
        "--task256-ready-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_ready_restart_with_escalate_monitoring_phase4_continuation_ready_summary_task256.json",
    )
    parser.add_argument(
        "--task256-escalate-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_ready_restart_with_escalate_monitoring_phase4_continuation_escalate_summary_task256.json",
    )
    parser.add_argument("--expected-prior-run-id", default="task256_20260305T050343Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task257_%Y%m%dT%H%M%SZ")
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    state_path = Path(args.state_latest_json)
    ready_rows = read_csv(Path(args.task256_ready_input_csv))
    escalate_rows = read_csv(Path(args.task256_escalate_input_csv))
    summary256_ready = read_json(Path(args.task256_ready_summary_json), default={})
    summary256_escalate = read_json(Path(args.task256_escalate_summary_json), default={})
    ...
    integrity_ready = dict(summary256_ready.get("integrity_checks") or {})
    integrity_escalate = dict(summary256_escalate.get("integrity_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task257_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "ready_restart_with_escalate_monitoring_phase5_continuation",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK257_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_ready_summary",
        "task": "TASK257",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task256_ready_input_csv": str(Path(args.task256_ready_input_csv)),
            "task256_ready_summary_json": str(Path(args.task256_ready_summary_json)),
        ...
        "next_task_recommendation": {
            "id": "TASK258",
            "title": "EXHIBITIONS-TEXT-STEADY-STATE-OPERATION-PROPOSAL-FOR-READY-AND-ESCALATE-LANES",
            "ja": "Define steady-state operation cadence and reintegration checkpoints for READY continuation and ESCALATE separate lane",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_escalate_summary",
        "task": "TASK257",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task256_escalate_input_csv": str(Path(args.task256_escalate_input_csv)),
            "task256_escalate_summary_json": str(Path(args.task256_escalate_summary_json)),
            "state_latest_json": str(state_path),
        },
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_ready_input_task257.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_ready_summary_task257.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_ready_manifest_task257.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_escalate_input_task257.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_escalate_summary_task257.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_escalate_manifest_task257.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_monitored_diff_task257.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_preflight_task257.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_backup_log_task257.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_by_fair_task257.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_by_gallery_task257.csv"
    )
    report_path = output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_task257.md"
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_ready_manifest",
        "task": "TASK257",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase5_continuation_escalate_manifest",
        "task": "TASK257",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK257 Exhibitions Text Ready Restart with Escalate Monitoring Phase5 Continuation",
        "",
        "## run",
