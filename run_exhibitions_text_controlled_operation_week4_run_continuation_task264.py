...
READY = "CONTROLLED_OPERATION_WEEK4_SUCCESS"
HOLD_ESCALATE = "HOLD_FOR_RUNBOOK_DEVIATION"
HOLD_BOUNDARY = "HOLD_FOR_RUNBOOK_DEVIATION"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK264 Exhibitions Text controlled operation week4 run and holding recheck scheduling"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument(
        "--task263-ready-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week3_run_and_holding_recheck_scheduling_ready_input_task263.csv",
    )
    parser.add_argument(
        "--task263-escalate-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week3_run_and_holding_recheck_scheduling_escalate_input_task263.csv",
    )
    parser.add_argument(
        "--task263-ready-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week3_run_and_holding_recheck_scheduling_ready_summary_task263.json",
    )
    parser.add_argument(
        "--task263-escalate-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week3_run_and_holding_recheck_scheduling_escalate_summary_task263.json",
    )
    parser.add_argument("--expected-prior-run-id", default="task263_20260305T075038Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task264_%Y%m%dT%H%M%SZ")
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    state_path = Path(args.state_latest_json)
    ready_rows = read_csv(Path(args.task263_ready_input_csv))
    escalate_rows = read_csv(Path(args.task263_escalate_input_csv))
    summary263_ready = read_json(Path(args.task263_ready_summary_json), default={})
    summary263_escalate = read_json(Path(args.task263_escalate_summary_json), default={})
    ...
    add_check("EXPECTED_PRIOR_RUN_ID_MATCH", last_ingested_run_id, str(args.expected_prior_run_id).strip())
    ...
    integrity_ready = dict(summary263_ready.get("common_checks") or {})
    integrity_escalate = dict(summary263_escalate.get("common_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task264_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "controlled_operation_week4_run_and_holding_recheck_scheduling",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK264_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_ready_summary",
        "task": "TASK264",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task263_ready_input_csv": str(Path(args.task263_ready_input_csv)),
            "task263_ready_summary_json": str(Path(args.task263_ready_summary_json)),
            "holding_set_csv": str(Path(args.holding_set_csv)),
            "reject_set_csv": str(Path(args.reject_set_csv)),
            "state_latest_json": str(state_path),
            "expected_prior_run_id": str(args.expected_prior_run_id).strip(),
        },
        "preflight": {
        ...
        "next_task_recommendation": {
            "id": "TASK265",
            "title": "EXHIBITIONS-TEXT-CONTROLLED-OPERATION-WEEK5-RUN-CONTINUATION",
            "ja": "Execute week-5 synchronized dual-lane run while keeping holding-lane readiness checks",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_escalate_summary",
        "task": "TASK264",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task263_escalate_input_csv": str(Path(args.task263_escalate_input_csv)),
            "task263_escalate_summary_json": str(Path(args.task263_escalate_summary_json)),
            "state_latest_json": str(state_path),
        },
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_ready_input_task264.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_ready_summary_task264.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_ready_manifest_task264.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_escalate_input_task264.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_escalate_summary_task264.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_escalate_manifest_task264.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_monitored_diff_task264.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_preflight_task264.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_backup_log_task264.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_by_fair_task264.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_by_gallery_task264.csv"
    )
    report_path = output_dir / "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_task264.md"
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_ready_manifest",
        "task": "TASK264",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week4_run_and_holding_recheck_scheduling_escalate_manifest",
        "task": "TASK264",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK264 Exhibitions Text controlled operation week4 run and holding recheck scheduling",
        "",
        "## run",
