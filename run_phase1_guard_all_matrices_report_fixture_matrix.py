...
DEFAULT_MANIFEST_PATH = "tests/fixtures/phase1_guard/report_fixture_manifest.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run report-CLI fixtures and validate expected exit codes + report summary checks."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help=f"directory to save matrix/report summaries (default: {DEFAULT_LOGS_DIR})",
    )
    parser.add_argument(
        "--output-path",
        default="",
        help=(
            "optional matrix summary output path "
            "(default: <logs-dir>/phase1_guard_all_matrices_report_fixture_matrix_<timestamp>.json)"
        ),
    )
    parser.add_argument(
        "--fail-fast",
...
def main() -> int:
    ...
    for index, raw_case in enumerate(raw_cases, start=1):
        ...
        output_report = logs_dir / REPORT_OUTPUT_TEMPLATE.format(
            case_name=sanitize_case_name(case_name),
            timestamp=case_timestamp,
        )

        required_fields_ok = bool(summary_path_raw) and expected_exit_code is not None
        ...
        summary_path = resolve_path(summary_path_raw, repo_root)
