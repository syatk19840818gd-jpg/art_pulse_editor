...
READY = "READY_FOR_READY_RESTART_WITH_ESCALATE_MONITORING_PHASE_3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK254 Exhibitions Text ready restart with escalate monitoring phase2 continuation"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument(
        "--task253-ready-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_ready_continuation_restart_phase2_input_task253.csv",
    )
    parser.add_argument(
        "--task253-escalate-input-csv",
        default="data/phase1_seed10/logs/exhibitions_text_escalate_lane_monitoring_phase2_input_task253.csv",
    )
    parser.add_argument(
        "--task253-ready-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_ready_continuation_restart_phase2_summary_task253.json",
    )
    parser.add_argument(
        "--task253-escalate-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_escalate_lane_monitoring_phase2_summary_task253.json",
    )
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task254_%Y%m%dT%H%M%SZ")
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    state_path = Path(args.state_latest_json)
    ready_rows = read_csv(Path(args.task253_ready_input_csv))
    escalate_rows = read_csv(Path(args.task253_escalate_input_csv))
    summary253_ready = read_json(Path(args.task253_ready_summary_json), default={})
    summary253_escalate = read_json(Path(args.task253_escalate_summary_json), default={})
    ...
    integrity_ready = dict(summary253_ready.get("integrity_checks") or {})
    integrity_escalate = dict(summary253_escalate.get("integrity_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task254_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "ready_restart_with_escalate_monitoring_phase2_continuation",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK254_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_ready_summary",
        "task": "TASK254",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task253_ready_input_csv": str(Path(args.task253_ready_input_csv)),
            "task253_ready_summary_json": str(Path(args.task253_ready_summary_json)),
        ...
        "next_task_recommendation": {
            "id": "TASK255",
            "title": "EXHIBITIONS-TEXT-READY-RESTART-WITH-ESCALATE-MONITORING-PHASE-3-CONTINUATION",
            "ja": "Continue READY restart cycle while keeping ESCALATE separate lane monitoring with strict boundaries",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_escalate_summary",
        "task": "TASK254",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task253_escalate_input_csv": str(Path(args.task253_escalate_input_csv)),
            "task253_escalate_summary_json": str(Path(args.task253_escalate_summary_json)),
            "state_latest_json": str(state_path),
        },
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_ready_input_task254.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_ready_summary_task254.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_ready_manifest_task254.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_escalate_input_task254.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_escalate_summary_task254.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_escalate_manifest_task254.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_monitored_diff_task254.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_preflight_task254.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_backup_log_task254.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_by_fair_task254.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_by_gallery_task254.csv"
    )
    report_path = output_dir / "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_task254.md"
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_ready_manifest",
        "task": "TASK254",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_ready_restart_with_escalate_monitoring_phase2_continuation_escalate_manifest",
        "task": "TASK254",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK254 Exhibitions Text Ready Restart with Escalate Monitoring Phase2 Continuation",
        "",
        "## run",
