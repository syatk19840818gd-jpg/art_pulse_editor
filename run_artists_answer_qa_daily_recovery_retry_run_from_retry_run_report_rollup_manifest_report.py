...
def resolve_latest_summary(search_dir: Path, pattern: str) -> tuple[Path | None, str | None]:
    ...
    candidates = [p for p in candidates if _is_summary_candidate(p)]
    if not candidates:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Read retry-run-from-rollup summary generated via retry-run-report-rollup manifest "
        ...
        if latest_error:
            ...
            output_path = (
                ...
                if args.output_json
                    ...
                    / "artists_answer_qa_daily_recovery_retry_run_from_rollup_manifest_report_latest_error.json"
                ).resolve()
            )
