...
READY = "CONTROLLED_OPERATION_WEEK2_SUCCESS"
HOLD_ESCALATE = "HOLD_FOR_HOLDING_RECHECK_SCHEDULING_GAP"
HOLD_BOUNDARY = "HOLD_FOR_HOLDING_RECHECK_SCHEDULING_GAP"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK262 Exhibitions Text controlled operation week2 run and holding recheck scheduling"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument("--expected-prior-run-id", default="task261_20260305T053811Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task262_%Y%m%dT%H%M%SZ")
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    state_path = Path(args.state_latest_json)
    ready_rows = read_csv(Path(args.task261_ready_input_csv))
    escalate_rows = read_csv(Path(args.task261_escalate_input_csv))
    summary261_ready = read_json(Path(args.task261_ready_summary_json), default={})
    summary261_escalate = read_json(Path(args.task261_escalate_summary_json), default={})
    ...
    integrity_ready = dict(summary261_ready.get("common_checks") or {})
    integrity_escalate = dict(summary261_escalate.get("common_checks") or {})
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task262_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "controlled_operation_week2_run_and_holding_recheck_scheduling",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK262_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_ready_summary",
        "task": "TASK262",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task261_ready_input_csv": str(Path(args.task261_ready_input_csv)),
            "task261_ready_summary_json": str(Path(args.task261_ready_summary_json)),
        ...
        "next_task_recommendation": {
            "id": "TASK263",
            "title": "EXHIBITIONS-TEXT-CONTROLLED-OPERATION-WEEK3-RUN-OR-HOLDING-RECHECK-PREPARATION",
            "ja": "Execute week-3 synchronized dual-lane run while maintaining holding-lane recheck schedule",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_escalate_summary",
        "task": "TASK262",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
            "task261_escalate_input_csv": str(Path(args.task261_escalate_input_csv)),
            "task261_escalate_summary_json": str(Path(args.task261_escalate_summary_json)),
            "state_latest_json": str(state_path),
        },
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_ready_input_task262.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_ready_summary_task262.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_ready_manifest_task262.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_escalate_input_task262.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_escalate_summary_task262.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_escalate_manifest_task262.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_monitored_diff_task262.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_preflight_task262.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_backup_log_task262.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_by_fair_task262.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_by_gallery_task262.csv"
    )
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_ready_manifest",
        "task": "TASK262",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week2_run_and_holding_recheck_scheduling_escalate_manifest",
        "task": "TASK262",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK262 Exhibitions Text Controlled Operation Week2 Run And Holding Recheck Scheduling",
        "",
        "## run",
