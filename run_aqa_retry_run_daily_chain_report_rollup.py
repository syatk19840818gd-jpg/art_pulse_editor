...
DEFAULT_LATEST_N = 20
SOURCE_CLI = "run_aqa_retry_run_daily_chain_report_rollup.py"
INPUT_ARTIFACT_KIND = "retry_run_daily_chain_report"
OUTPUT_ARTIFACT_KIND = "retry_run_daily_chain_report_rollup"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Roll up artists retry-run daily chain reports and extract failed runs."
    )
    parser.add_argument(
        "--search-dir",
...
def main() -> int:
    ...
    output_path = (
        ...
        if args.output_json
            ...
            search_dir
            / f"artists_answer_qa_retry_run_daily_chain_report_rollup_{utc_timestamp_compact()}.json"
        ).resolve()
    )
    ...
    if not report_paths:
        message = f"daily_chain_reports_not_found:{search_dir}:{args.glob or '[default_glob]'}"
        rollup["warnings"].append(message)
        write_json(output_path, rollup)
        print(f"[ERROR] {message}")
        print(f"[DONE] rollup={output_path}")
