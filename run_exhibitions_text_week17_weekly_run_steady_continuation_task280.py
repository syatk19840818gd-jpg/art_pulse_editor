...
SUCCESS = "CONTROLLED_OPERATION_WEEK17_SUCCESS"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TASK280 week17 steady continuation run")
    p.add_argument("--state-latest-json", default="data/phase1_seed10/logs/exhibitions_text_monitored_state_latest.json")
    p.add_argument("--ready-input-csv", default="data/phase1_seed10/logs/exhibitions_text_week16_weekly_run_steady_continuation_ready_input_task279.csv")
    p.add_argument("--escalate-input-csv", default="data/phase1_seed10/logs/exhibitions_text_week16_weekly_run_steady_continuation_escalate_input_task279.csv")
    ...
    p.add_argument("--task279-summary-json", default="data/phase1_seed10/logs/exhibitions_text_week16_weekly_run_steady_continuation_summary_task279.json")
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task280_%Y%m%dT%H%M%SZ")
    ...
    summary279 = read_json(Path(args.task279_summary_json), default={})

    expected_prior_run_id = str(summary279.get("run_id") or "").strip()
    summary279_ready = str(summary279.get("go_hold_decision") or "").strip() == "CONTROLLED_OPERATION_WEEK16_SUCCESS"
    ...
    add_check("TASK279_WEEK16_SUCCESS", summary279_ready, True)
    ...
    common279 = dict(dict(summary279.get("post_run_verification") or {}).get("common") or {})
    coverage_review_count = int(common279.get("coverage_review_count", 0) or 0)
    reject_candidate_count = int(common279.get("reject_candidate_count", 0) or 0)
    join_blocker_count = int(common279.get("join_blocker_count", 0) or 0)
    ...
    if blocker_fail_count == 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task280_week17_steady_{run_id}"
    ...
    if blocker_fail_count == 0 and backup_created and backup_hash_match:
        ...
        out["last_ingestion"] = {
            "task_run_id": run_id,
            "ingestion_mode": "controlled_operation_week17_steady_continuation",
    ...
    ready_summary = {
        "artifact": "exhibitions_text_week17_weekly_run_steady_continuation_ready_summary_task280",
        "task": "TASK280",
        "run_id": run_id,
    ...
    escalate_summary = {
        "artifact": "exhibitions_text_week17_weekly_run_steady_continuation_escalate_summary_task280",
        "task": "TASK280",
        "run_id": run_id,
        "created_at": utc_now_iso(),
    ...
    post = {
        "artifact": "exhibitions_text_week17_weekly_run_steady_continuation_post_run_verification_task280",
        "task": "TASK280",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "monitored_ready": {"monitoring_required_count": monitored_ready_count, "evaluated_count": len(monitored_ready_eval_rows), "policy_breach_count": mr_lane_hold, "evaluation_evidence_file": "exhibitions_text_week17_weekly_run_steady_continuation_monitored_ready_eval_task280.csv"},
    ...
    summary = {
        "artifact": "exhibitions_text_week17_weekly_run_steady_continuation_summary_task280",
        "task": "TASK280",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "go_hold_decision": decision,
        "blocker_labels": blocker_labels,
        "next_task_recommendation": {"id": "TASK281", "title": "EXHIBITIONS-TEXT-WEEK18-WEEKLY-RUN-STEADY-CONTINUATION", "ja": "Run Week18 steady weekly sync with reflected lanes and monitored-ready tracking continuity"},
    }

    base = output_dir
    ready_input_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_ready_input_task280.csv"
    ready_summary_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_ready_summary_task280.json"
    ready_manifest_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_ready_manifest_task280.json"
    esc_input_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_escalate_input_task280.csv"
    esc_summary_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_escalate_summary_task280.json"
    esc_manifest_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_escalate_manifest_task280.json"
    preflight_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_preflight_task280.csv"
    backup_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_backup_log_task280.json"
    diff_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_monitored_diff_task280.csv"
    mr_eval_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_monitored_ready_eval_task280.csv"
    by_fair_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_by_fair_task280.csv"
    by_gallery_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_by_gallery_task280.csv"
    post_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_post_run_verification_task280.json"
    stage_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_runbook_stage_result_task280.csv"
    summary_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_summary_task280.json"
    manifest_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_manifest_task280.json"
    report_out = base / "exhibitions_text_week17_weekly_run_steady_continuation_task280.md"
