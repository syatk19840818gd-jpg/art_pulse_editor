...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Finalize TASK266 week6 run and holding recheck readiness artifacts"
    )
    parser.add_argument(
        "--ready-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_summary_task266.json",
    )
    parser.add_argument(
        "--escalate-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_summary_task266.json",
    )
    parser.add_argument(
        "--preflight-csv",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_preflight_task266.csv",
    )
    parser.add_argument(
        "--backup-log-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_backup_log_task266.json",
    )
    parser.add_argument(
        "--ready-manifest-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_ready_manifest_task266.json",
    )
    parser.add_argument(
        "--escalate-manifest-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_escalate_manifest_task266.json",
    )
    parser.add_argument(
        "--holding-set-csv",
    ...
    parser.add_argument(
        ...
        default="data/phase1_seed10/logs/holding_recheck_readiness_task265.json",
    )
    parser.add_argument(
        "--previous-week-summary-json",
        default="data/phase1_seed10/logs/exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_summary_task265.json",
    )
...
def main() -> int:
    ...
    add_stage(
        ...
        (not bool(restore.get("restore_executed", False))) and decision == "CONTROLLED_OPERATION_WEEK6_SUCCESS",
    ...
    holding_recheck_readiness = {
        "artifact": "holding_recheck_readiness_task266",
        "task": "TASK266",
        "run_id": run_id,
        ...
        "recheck_target_bundle_policy": {
            ...
            "execution_note": "readiness check only in TASK266; no holding re-evaluation execution in this run",
        },
        "conclusion": holding_readiness_conclusion,
    }

    stage8_passed = len(holding_rows) == 17 and holding_readiness_conclusion in {
        "HOLDING_RECHECK_NOT_DUE",
        "HOLDING_RECHECK_DUE",
    }
    add_stage(
        8,
    ...
    if decision == "CONTROLLED_OPERATION_WEEK6_SUCCESS":
        holding_readiness_valid = len(holding_rows) == 17 and bool(next_recheck_trigger_run)
        if not holding_readiness_valid:
            decision = "HOLD_FOR_HOLDING_RECHECK_TRIGGER_CONFLICT"
        elif holding_recheck_due:
            decision = "CONTROLLED_OPERATION_WEEK6_SUCCESS_HOLDING_DUE"
        else:
            decision = "CONTROLLED_OPERATION_WEEK6_SUCCESS_HOLDING_NOT_DUE"
    ...
    post_run_verification = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_post_run_verification",
        "task": "TASK266",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "ready_side": {
    ...
    summary = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_summary",
        "task": "TASK266",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "initial_start": False,
        "weekly_run_phase": "WEEK6",
        ...
        "next_task_recommendation": {
            "id": "TASK267",
            "title": "EXHIBITIONS-TEXT-HOLDING-RECHECK-PREP-OR-WEEK7-CONTINUATION-DECISION",
            "ja": "Proceed to holding recheck preparation if due; otherwise continue week7 operation",
        },
    }

    summary_path = output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_summary_task266.json"
    post_verification_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_post_run_verification_task266.json"
    )
    stage_csv_path = (
        output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_runbook_stage_result_task266.csv"
    )
    holding_readiness_json_path = output_dir / "holding_recheck_readiness_task266.json"
    holding_readiness_csv_path = output_dir / "holding_recheck_readiness_task266.csv"
    manifest_path = output_dir / "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_manifest_task266.json"
    ...
    manifest = {
        "artifact": "exhibitions_text_controlled_operation_week6_run_and_holding_recheck_scheduling_manifest",
        "task": "TASK266",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "decision": decision,
    ...
    report_lines = [
        "# TASK266 Controlled Operation Week6 Run And Holding Recheck Readiness",
        "",
        f"- run_id={run_id}",
        f"- decision={decision}",
        "",
