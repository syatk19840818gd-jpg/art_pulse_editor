...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate trial result bundle for QA handoff readiness "
            "(no actual QA/adoption execution)."
        )
    )
    parser.add_argument("--trial-execution-manifest", default="")
    parser.add_argument("--trial-execution-summary", default="")
    parser.add_argument("--trial-execution-report", default="")
    parser.add_argument("--trial-bundle-result-csv", default="")
    parser.add_argument("--trial-qa-handoff-manifest", default="")

    parser.add_argument("--trial-unit-result-root", default="")
    parser.add_argument("--trial-failure-queue-csv", default="")
    parser.add_argument("--trial-defer-queue-csv", default="")

    parser.add_argument("--trial-runtime-input-manifest-json", default="")
    parser.add_argument("--handoff-validation-manifest-json", default="")
...
def build_output_paths(output_dir: Path, run_id: str) -> dict[str, Path]:
    return {
        ...
        / f"exhibitions_image_task_t203_qa_handoff_validation_summary_{run_id}.json",
        "report_md": output_dir
        / f"exhibitions_image_task_t203_qa_handoff_validation_report_{run_id}.md",
        "manifest_json": output_dir
        / f"exhibitions_image_task_t203_qa_handoff_validation_manifest_{run_id}.json",
        "errors_csv": output_dir
        / f"exhibitions_image_task_t203_qa_handoff_validation_errors_{run_id}.csv",
        "warnings_csv": output_dir
        / f"exhibitions_image_task_t203_qa_handoff_validation_warnings_{run_id}.csv",
    }
...
def resolve_input_paths(args: argparse.Namespace) -> dict[str, str]:
    resolved = {
        ...
        "trial_execution_report_md": norm_text(args.trial_execution_report),
    ...
    manifest_path = resolved["trial_execution_manifest_json"]
    if manifest_path and Path(manifest_path).exists():
        payload = read_json(Path(manifest_path))
        outputs = payload.get("outputs") or {}
        if not resolved["trial_execution_summary_json"]:
            resolved["trial_execution_summary_json"] = norm_text(outputs.get("summary_json"))
        if not resolved["trial_execution_report_md"]:
            resolved["trial_execution_report_md"] = norm_text(outputs.get("report_md"))
        if not resolved["trial_bundle_result_csv"]:
            resolved["trial_bundle_result_csv"] = norm_text(outputs.get("bundle_result_csv"))
        if not resolved["trial_qa_handoff_manifest_json"]:
            resolved["trial_qa_handoff_manifest_json"] = norm_text(
                outputs.get("qa_handoff_manifest_json")
            )
        if not resolved["trial_failure_queue_csv"]:
            resolved["trial_failure_queue_csv"] = norm_text(outputs.get("failure_queue_csv"))
        if not resolved["trial_defer_queue_csv"]:
            resolved["trial_defer_queue_csv"] = norm_text(outputs.get("defer_queue_csv"))
        inputs = payload.get("inputs") or {}
        if not resolved["trial_runtime_input_manifest_json"]:
            resolved["trial_runtime_input_manifest_json"] = norm_text(
                inputs.get("trial_runtime_input_manifest_json")
            )
    return resolved
...
def validate_summary_fields(
    ...
    trial_runner_status = norm_text(summary.get("trial_runner_status"))
    qa_handoff_allowed = parse_bool(summary.get("qa_handoff_allowed"))
    ...
    if trial_runner_status not in {RUNNER_STATUS_SUCCESS, RUNNER_STATUS_PARTIAL}:
        state.add_error(
            code="trial_runner_status_not_qa_eligible",
            check_id="summary_gate",
            message=f"trial_runner_status must be SUCCESS|PARTIAL_SUCCESS, got={trial_runner_status}",
            category="consistency",
            file=str(summary_path),
        )
    if qa_handoff_allowed is not True:
        state.add_error(
            code="qa_handoff_not_allowed",
            check_id="summary_gate",
            message="qa_handoff_allowed must be true",
            category="consistency",
            file=str(summary_path),
        )
    ...
    return {
        "trial_runner_status": trial_runner_status,
...
def main() -> int:
    args = parse_args()
    run_id = norm_text(args.run_id) or f"{utc_compact()}-qa-handoff-validate"
    ...
    try:
        if norm_text(os.environ.get("TASK203_FORCE_INTERNAL_FAILURE")) in {"1", "true", "yes"}:
            raise RuntimeError("forced_internal_failure_for_task203")
        ...
        summary_path = require_file(
            resolved["trial_execution_summary_json"],
            label="trial_execution_summary_json",
            state=state,
        )
        report_path = require_file(
            resolved["trial_execution_report_md"],
            label="trial_execution_report_md",
            state=state,
        )
        bundle_result_path = require_file(
            resolved["trial_bundle_result_csv"],
            label="trial_bundle_result_csv",
            state=state,
        )
        qa_handoff_manifest_path = require_file(
            resolved["trial_qa_handoff_manifest_json"],
            label="trial_qa_handoff_manifest_json",
            state=state,
        )

        trial_runtime_input_manifest_path: Path | None = None
        handoff_validation_manifest_path: Path | None = None
        classification_bundle_manifest_path: Path | None = None
        if args.strict_trace:
            trial_runtime_input_manifest_path = require_file(
                resolved["trial_runtime_input_manifest_json"],
                label="trial_runtime_input_manifest_json",
                state=state,
            )
            handoff_validation_manifest_path = require_file(
                resolved["handoff_validation_manifest_json"],
            ...
            if norm_text(resolved["trial_runtime_input_manifest_json"]):
                trial_runtime_input_manifest_path = require_file(
                    resolved["trial_runtime_input_manifest_json"],
                    label="trial_runtime_input_manifest_json",
                    state=state,
                )
            if norm_text(resolved["handoff_validation_manifest_json"]):
                handoff_validation_manifest_path = require_file(
                    resolved["handoff_validation_manifest_json"],
                    label="handoff_validation_manifest_json",
                    state=state,
        ...
        if state.errors:
            ...
            summary_payload = {
                ...
                "qa_handoff_verdict": VERDICT_FAIL,
            ...
            write_md(
                ...
                [
                    "# TASK203 QA Handoff Validation Report",
                    "",
                    f"- qa_handoff_verdict: `{VERDICT_FAIL}`",
        ...
        assert (
            ...
            and qa_handoff_manifest_path
        )
        trial_manifest = read_json(manifest_path)
        trial_summary = read_json(summary_path)
        qa_handoff_manifest = read_json(qa_handoff_manifest_path)
        bundle_fields, bundle_rows = read_csv(bundle_result_path)
        summary_info = validate_summary_fields(trial_summary, state, summary_path)
        ...
        if not bundle_rows:
            state.add_error(
                ...
                message="trial_bundle_result.csv has no rows",
                category="impossible_set",
        ...
        for row in bundle_rows:
            ...
            if unit_id in seen_unit_ids:
                state.add_error(
                    ...
                    check_id="bundle_result_pk",
                    message=f"duplicate planned_unit_id in trial_bundle_result: {unit_id}",
                    category="consistency",
                    file=str(bundle_result_path),
                    row_key=row_key,
                )
            ...
            unit_rows_path = norm_text(row.get("trial_unit_rows_csv_path"))
            if not unit_json_path and norm_text(args.trial_unit_result_root):
                unit_json_path = str(
                    Path(args.trial_unit_result_root) / f"t201_ures_{unit_id}.json"
                )

            unit_json = require_file(
                unit_json_path,
                label=f"trial_unit_result_json:{unit_id}",
                state=state,
            )
            unit_rows_csv = require_file(
                unit_rows_path,
                label=f"trial_unit_rows_csv:{unit_id}",
                state=state,
            )
                ...
                if unit_exec != len(unit_rows):
                    state.add_error(
                        ...
                        message=(
                            ...
                            f"but trial_unit_rows rows={len(unit_rows)}"
                        ),
                        category="consistency",
            ...
            unit_results.append(
                {
                    ...
                    "trial_unit_result_json_path": str(unit_json),
                    "trial_unit_rows_csv_path": str(unit_rows_csv),
                }
            )
        ...
        handoff_status = norm_text(qa_handoff_manifest.get("trial_runner_status"))
        handoff_allowed = parse_bool(qa_handoff_manifest.get("qa_handoff_allowed"))
        if handoff_status not in {RUNNER_STATUS_SUCCESS, RUNNER_STATUS_PARTIAL}:
            state.add_error(
                ...
                message=f"trial_runner_status in qa_handoff_manifest must be SUCCESS|PARTIAL_SUCCESS, got={handoff_status}",
                category="consistency",
                file=str(qa_handoff_manifest_path),
            )
        if handoff_allowed is not True:
            state.add_error(
                code="handoff_manifest_not_allowed",
                check_id="handoff_manifest_gate",
                message="qa_handoff_allowed in qa_handoff_manifest must be true",
                category="consistency",
                file=str(qa_handoff_manifest_path),
            )

        handoff_executed_units = set(as_text_list(qa_handoff_manifest.get("executed_unit_ids")))
        if not handoff_executed_units:
            state.add_error(
                code="handoff_manifest_empty_executed_units",
                check_id="handoff_manifest_unit_set",
                message="qa_handoff_manifest executed_unit_ids is empty",
                category="impossible_set",
                file=str(qa_handoff_manifest_path),
            )
        if handoff_executed_units and handoff_executed_units != bundle_unit_ids:
            state.add_error(
                ...
                check_id="handoff_manifest_unit_set",
                message=(
                    "qa_handoff_manifest executed_unit_ids mismatch bundle units: "
        ...
        unit_result_paths = [Path(v) for v in as_text_list(qa_handoff_manifest.get("unit_result_paths"))]
        ...
        for path in unit_result_paths:
            if not path.exists():
                state.add_error(
                    ...
                    message=f"qa_handoff_manifest references missing unit_result_path: {path}",
                    category="missing_artifact",
                    file=str(path),
                )

        bundle_summary_path = Path(norm_text(qa_handoff_manifest.get("bundle_summary_path")))
        if not bundle_summary_path.exists():
            state.add_error(
                code="handoff_manifest_bundle_summary_missing",
                check_id="handoff_manifest_output_paths",
                message=f"qa_handoff_manifest references missing bundle_summary_path: {bundle_summary_path}",
                category="missing_artifact",
                file=str(bundle_summary_path),
            )

        failure_queue_path = norm_text(resolved.get("trial_failure_queue_csv")) or norm_text(
            qa_handoff_manifest.get("failure_queue_path")
        )
        defer_queue_path = norm_text(resolved.get("trial_defer_queue_csv")) or norm_text(
            qa_handoff_manifest.get("defer_queue_path")
        )
        ...
        if summary_info["failed_unit_count"] > 0:
            failure_path_obj = require_file(
                failure_queue_path,
                label="trial_failure_queue_csv(required because failed_unit_count>0)",
                state=state,
            )
            if failure_path_obj:
                ...
                ensure_required_columns(
                    failure_fields,
                    ["planned_unit_id", "actual_trial_run_id", "failed_row_count"],
            ...
            if maybe_failure.exists():
                state.add_warning(
                    ...
                    message="trial_failure_queue.csv exists though failed_unit_count=0",
        ...
        if summary_info["deferred_unit_count"] > 0:
            defer_path_obj = require_file(
                defer_queue_path,
                label="trial_defer_queue_csv(required because deferred_unit_count>0)",
                state=state,
            )
            if defer_path_obj:
                ...
                ensure_required_columns(
                    defer_fields,
                    ["planned_unit_id", "actual_trial_run_id", "deferred_row_count"],
            ...
            if maybe_defer.exists():
                state.add_warning(
                    ...
                    check_id="optional_queue_presence",
                    message="trial_defer_queue.csv exists though deferred_unit_count=0",
        ...
        handoff_ids = {
            "classification_run_id": norm_text(qa_handoff_manifest.get("classification_run_id")),
            "bundle_id": norm_text(qa_handoff_manifest.get("bundle_id")),
            "trial_runner_bundle_id": norm_text(qa_handoff_manifest.get("trial_runner_bundle_id")),
            "scope_hash": norm_text(qa_handoff_manifest.get("scope_hash")),
        }
        for key in ["classification_run_id", "bundle_id", "trial_runner_bundle_id", "scope_hash"]:
        ...
        if (
            unit_summary_runner_bundle_ids
            and unit_summary_runner_bundle_ids != {summary_info["trial_runner_bundle_id"]}
        ):
            state.add_error(
                code="cross_file_trial_runner_bundle_id_mismatch",
                check_id="cross_file_ids",
                message=f"trial_runner_bundle_id mismatch in per-unit results: {sorted(unit_summary_runner_bundle_ids)} vs summary={summary_info['trial_runner_bundle_id']}",
                category="consistency",
                file=str(bundle_result_path),
            )

        handoff_fairs = set(as_text_list(qa_handoff_manifest.get("fair_slug_set")))
        handoff_lanes = set(as_text_list(qa_handoff_manifest.get("lane_set")))
        handoff_years = set(as_text_list(qa_handoff_manifest.get("target_year")))
        ...
        if trial_runtime_input_manifest_path:
            runtime_payload = read_json(trial_runtime_input_manifest_path)
            ...
            if runtime_classification and runtime_classification != summary_info["classification_run_id"]:
                state.add_error(
                    ...
                    category="consistency",
                    file=str(trial_runtime_input_manifest_path),
                )
        ...
        if handoff_validation_manifest_path:
            handoff_validation_payload = read_json(handoff_validation_manifest_path)
            handoff_verdict = norm_text(handoff_validation_payload.get("handoff_verdict"))
            if handoff_verdict and handoff_verdict != "PASS":
                state.add_error(
                    ...
                    category="consistency",
                    file=str(handoff_validation_manifest_path),
                )
        ...
        if state.errors:
            ...
            exit_code = choose_fail_exit(state.errors)
            qa_handoff_allowed_final = False
            manual_review_required = True
            retry_recommended = True
        else:
            hold_reasons: list[str] = []
            if summary_info["trial_runner_status"] == RUNNER_STATUS_PARTIAL:
                hold_reasons.append("trial_runner_status_partial_success")
        ...
        summary_payload = {
            ...
            "started_at": started_at,
            "completed_at": utc_now_iso(),
            "qa_handoff_verdict": verdict,
            "qa_handoff_allowed": qa_handoff_allowed_final,
            ...
            "exit_code": exit_code,
            "output_paths": {k: str(v) for k, v in output_paths.items()},
            "qa_minimum_handoff_set": {
                "trial_qa_handoff_manifest_json": str(qa_handoff_manifest_path),
                "trial_execution_summary_json": str(summary_path),
                "trial_bundle_result_csv": str(bundle_result_path),
                "trial_unit_result_json_paths": [
                    u["trial_unit_result_json_path"] for u in selected_units
                ],
                "trial_unit_rows_csv_paths": [
                    u["trial_unit_rows_csv_path"] for u in selected_units
                ],
                "trial_failure_queue_csv": failure_queue_path,
                "trial_defer_queue_csv": defer_queue_path,
                "scope_hash": summary_info["scope_hash"],
        ...
        write_json(
            output_paths["manifest_json"],
            {
                ...
                "inputs": resolved,
                "outputs": {k: str(v) for k, v in output_paths.items()},
                "qa_minimum_handoff_set": summary_payload["qa_minimum_handoff_set"],
            },
        )
        ...
        report_lines.extend(
            [
                "",
                "## Next Action",
                "- PASS: proceed to post-trial QA execution stage",
