...
READY = "CONTROLLED_OPERATION_WEEK5_SUCCESS"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TASK265 Exhibitions Text controlled operation week5 run and holding recheck scheduling"
    )
    parser.add_argument(
        "--state-latest-json",
    ...
    parser.add_argument("--expected-prior-run-id", default="task264_20260305T075846Z")
    parser.add_argument(
        "--holding-set-csv",
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task265_%Y%m%dT%H%M%SZ")
    ...
    if blocker_fail_count > 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task265_live_monitored_state_{run_id}"
        ...
        if not backup_created or not backup_hash_match:
            ...
            try:
                ...
                state["last_ingestion"] = {
                    "task_run_id": run_id,
                    "ingestion_mode": "controlled_operation_week5_run_and_holding_recheck_scheduling",
                ...
                if backup_created and backup_state_path.exists():
                    ...
                    restore_succeeded = file_sha256(state_path) == backup_hash
                decision = HOLD_RESTORE
                blocker_labels.append("TASK265_UPDATE_EXCEPTION")
    ...
    ready_summary = {
        "artifact": "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_summary",
        "task": "TASK265",
        "run_id": run_id,
        ...
        "next_task_recommendation": {
            ...
            "ja": "Execute week-6 synchronized dual-lane run and prepare for task266 holding trigger boundary",
        },
    }

    escalate_summary = {
        "artifact": "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_summary",
        "task": "TASK265",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        "inputs": {
    ...
    ready_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_input_task265.csv"
    )
    ready_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_summary_task265.json"
    )
    ready_manifest_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_manifest_task265.json"
    )
    escalate_input_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_input_task265.csv"
    )
    escalate_summary_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_summary_task265.json"
    )
    escalate_manifest_path = (
        output_dir
        / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_manifest_task265.json"
    )
    monitored_diff_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_monitored_diff_task265.csv"
    )
    preflight_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_preflight_task265.csv"
    )
    backup_log_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_backup_log_task265.json"
    )
    by_fair_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_by_fair_task265.csv"
    )
    by_gallery_path = (
        output_dir / "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_by_gallery_task265.csv"
    )
    ...
    ready_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_ready_manifest",
        "task": "TASK265",
        "run_id": run_id,
    ...
    escalate_manifest = {
        "artifact": "exhibitions_text_controlled_operation_week5_run_and_holding_recheck_scheduling_escalate_manifest",
        "task": "TASK265",
        "run_id": run_id,
    ...
    report_lines = [
        "# TASK265 Exhibitions Text controlled operation week5 run and holding recheck scheduling",
        "",
        "## run",
