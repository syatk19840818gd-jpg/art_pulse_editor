...
DEFAULT_GLOB = "artists_answer_qa_retry_run_daily_chain_summary_*.json"
SOURCE_CLI = "run_aqa_retry_run_daily_chain_report.py"
INPUT_ARTIFACT_KIND = "retry_run_daily_chain_summary"
OUTPUT_ARTIFACT_KIND = "retry_run_daily_chain_report"
...
def _collect_child_summary_paths(
    ...
    if not collected:
        top_output_paths = summary.get("output_paths")
        if isinstance(top_output_paths, dict):
            for value in top_output_paths.values():
                add_path(value)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read retry-run daily chain summary and write a lightweight report."
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--summary-path", default="", help="daily chain summary path")
    group.add_argument("--latest", action="store_true", help="resolve latest daily chain summary")
    parser.add_argument(
        "--search-dir",
...
def main() -> int:
    ...
    if args.summary_path:
        ...
        if latest_error:
            ...
            output_path = (
                ...
                if args.output_json
                else (Path(args.search_dir).resolve() / "artists_answer_qa_retry_run_daily_chain_report_latest_error.json")
            )
    ...
    if not summary_path.exists():
        error = f"daily_chain_summary_not_found:{summary_path}"
    ...
    try:
        ...
        error = f"daily_chain_summary_load_failed:{exc}"
        report["notes"].append(error)
        print(f"[ERROR] {error}")
        write_json(output_path, report)
        print(f"[DONE] report={output_path}")
