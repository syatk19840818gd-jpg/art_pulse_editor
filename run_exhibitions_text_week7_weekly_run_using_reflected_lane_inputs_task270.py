...
SUCCESS = "CONTROLLED_OPERATION_WEEK7_SUCCESS_WITH_REFLECTED_LANES"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TASK270 week7 reflected lane run")
    p.add_argument("--state-latest-json", default="data/phase1_seed10/logs/exhibitions_text_monitored_state_latest.json")
    p.add_argument("--ready-input-csv", default="data/phase1_seed10/logs/weekly_ready_input_next_task269.csv")
    p.add_argument("--escalate-input-csv", default="data/phase1_seed10/logs/weekly_escalate_input_next_task269.csv")
    ...
    p.add_argument("--task269-summary-json", default="data/phase1_seed10/logs/recheck_lane_reflection_policy_summary_task269.json")
    p.add_argument("--task268-summary-json", default="data/phase1_seed10/logs/holding_recheck_run_summary_task268.json")
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task270_%Y%m%dT%H%M%SZ")
    ...
    summary269 = read_json(Path(args.task269_summary_json), default={})
    summary268 = read_json(Path(args.task268_summary_json), default={})

    expected_prior_run_id = str(summary268.get("run_id") or "").strip()
    summary269_ready = str(summary269.get("decision") or "").strip() == "READY_FOR_WEEKLY_RUN_WITH_REFLECTED_LANES"
    ...
    add_check("TASK269_REFLECTION_READY", summary269_ready, True)
    ...
    common268 = dict(dict(summary268.get("post_run_verification") or {}).get("common") or {})
    coverage_review_count = int(common268.get("coverage_review_count", 0) or 0)
    reject_candidate_count = int(common268.get("reject_candidate_count", 0) or 0)
    join_blocker_count = int(common268.get("join_blocker_count", 0) or 0)
    ...
    if blocker_fail_count == 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task270_week7_reflected_{run_id}"
    ...
    if blocker_fail_count == 0 and backup_created and backup_hash_match:
        ...
        out["last_ingestion"] = {
            "task_run_id": run_id,
            "ingestion_mode": "controlled_operation_week7_reflected_lane_run",
    ...
    ready_summary = {
        "artifact": "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_ready_summary_task270",
        "task": "TASK270",
        "run_id": run_id,
    ...
    escalate_summary = {
        "artifact": "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_escalate_summary_task270",
        "task": "TASK270",
        "run_id": run_id,
        "created_at": utc_now_iso(),
    ...
    post = {
        "artifact": "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_post_run_verification_task270",
        "task": "TASK270",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "monitored_ready": {"monitoring_required_count": monitored_ready_count, "evaluated_count": len(monitored_ready_eval_rows), "policy_breach_count": mr_lane_hold, "evaluation_evidence_file": "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_monitored_ready_eval_task270.csv"},
    ...
    summary = {
        "artifact": "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_summary_task270",
        "task": "TASK270",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "go_hold_decision": decision,
        "blocker_labels": blocker_labels,
        "next_task_recommendation": {"id": "TASK271", "title": "EXHIBITIONS-TEXT-WEEK8-WEEKLY-RUN-STEADY-CONTINUATION", "ja": "Run Week8 steady weekly sync with reflected lanes and monitored-ready tracking continuity"},
    }

    base = output_dir
    ready_input_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_ready_input_task270.csv"
    ready_summary_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_ready_summary_task270.json"
    ready_manifest_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_ready_manifest_task270.json"
    esc_input_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_escalate_input_task270.csv"
    esc_summary_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_escalate_summary_task270.json"
    esc_manifest_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_escalate_manifest_task270.json"
    preflight_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_preflight_task270.csv"
    backup_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_backup_log_task270.json"
    diff_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_monitored_diff_task270.csv"
    mr_eval_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_monitored_ready_eval_task270.csv"
    by_fair_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_by_fair_task270.csv"
    by_gallery_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_by_gallery_task270.csv"
    post_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_post_run_verification_task270.json"
    stage_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_runbook_stage_result_task270.csv"
    summary_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_summary_task270.json"
    manifest_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_manifest_task270.json"
    report_out = base / "exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_task270.md"
    ...
    write_json(ready_manifest_out, {"artifact":"exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_ready_manifest_task270","task":"TASK270","run_id":run_id,"decision":decision})
    write_json(esc_manifest_out, {"artifact":"exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_escalate_manifest_task270","task":"TASK270","run_id":run_id,"decision":decision})
    write_json(manifest_out, {"artifact":"exhibitions_text_week7_weekly_run_using_reflected_lane_inputs_manifest_task270","task":"TASK270","run_id":run_id,"created_at":utc_now_iso(),"decision":decision,"outputs":{"summary_json":str(summary_out),"preflight_csv":str(preflight_out),"backup_log_json":str(backup_out),"ready_summary_json":str(ready_summary_out),"ready_manifest_json":str(ready_manifest_out),"ready_input_csv":str(ready_input_out),"escalate_summary_json":str(esc_summary_out),"escalate_manifest_json":str(esc_manifest_out),"escalate_input_csv":str(esc_input_out),"post_run_verification_json":str(post_out),"runbook_stage_result_csv":str(stage_out),"monitored_diff_csv":str(diff_out),"monitored_ready_eval_csv":str(mr_eval_out),"by_fair_csv":str(by_fair_out),"by_gallery_csv":str(by_gallery_out),"report_md":str(report_out)}})

    report_out.write_text("\n".join([
        "# TASK270 Week7 Weekly Run Using Reflected Lane Inputs",
        f"- run_id={run_id}",
        f"- decision={decision}",
    ...
    print(f"[task270] run_id={run_id} ready={len(ready_rows)} escalate={len(escalate_rows)} holding_excluded={len(holding_rows)} monitored_ready={monitored_ready_count} monitored_hold={mr_lane_hold} boundary={boundary_breach_count} gap={temporal_gap_count} escalate_blocker={escalate_blocker_count} restore={restore_executed} decision={decision}")
    return 0
