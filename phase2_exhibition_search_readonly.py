...
from phase2_common_readonly import (
    ...
    FAIR_SLUG_TO_LABEL,
    normalize_url,
    resolve_current_exhibitions_enrichment_output_paths_by_year,
    resolve_current_exhibitions_image_meta_paths_by_year,
    resolve_current_exhibitions_text_paths_by_year,
    safe_load_jsonl,
)
...
def _load_exhibition_enrichment_map_all_years() -> Tuple[Dict[str, Dict[str, str]], List[str]]:
    enrichment_by_source: Dict[str, Dict[str, str]] = {}
    warnings: List[str] = []
    for _year, source_path in sorted(
        resolve_current_exhibitions_enrichment_output_paths_by_year().items(),
        reverse=True,
    ):
        rows, path_warnings = safe_load_jsonl(source_path)
...
def load_exhibition_records_readonly() -> ExhibitionSearchData:
    ...
    enrichment_by_source, enrichment_warnings = _load_exhibition_enrichment_map_all_years()
    warnings.extend(enrichment_warnings)

    image_count_by_source: Dict[str, int] = {}
    image_preview_by_source: Dict[str, str] = {}
    image_preview_r2_by_source: Dict[str, str] = {}
    for fair_slug, paths_by_year in resolve_current_exhibitions_image_meta_paths_by_year().items():
        for _year, meta_path in sorted(paths_by_year.items(), reverse=True):
    ...
    for fair_slug, paths_by_year in resolve_current_exhibitions_text_paths_by_year().items():
        for discovered_year, text_path in sorted(paths_by_year.items()):
            text_rows, text_warnings = safe_load_jsonl(text_path)
            warnings.extend(text_warnings)
            fair_rows[fair_slug] = fair_rows.get(fair_slug, 0) + len(text_rows)
            ...
            for idx, row in enumerate(text_rows, start=1):
                ...
                record = {
                    "id": f"{fair_slug}:{year or discovered_year}:{idx}",
                    "fair_slug": fair_slug,
    ...
    return ExhibitionSearchData(
        ...
        fair_rows=fair_rows,
        count_note=(
            "Exhibition rows aggregate all available year-scoped current raw files "
            "(exhibitions_*_{year}.jsonl). "
            "summary_ja/headline_ja uses all available current Exhibition enrichment outputs "
            "(data/current/enrichment/exhibitions_enrichment_apply_output_{year}.jsonl), "
            "with strict current-only resolution. "
            "history is not used as a default query path. "
            "image_count_hint is read-only matched by source_url using all available year-scoped Exhibition image metadata."
        ),
    )
