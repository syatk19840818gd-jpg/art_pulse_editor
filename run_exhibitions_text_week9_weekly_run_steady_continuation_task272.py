...
SUCCESS = "CONTROLLED_OPERATION_WEEK9_SUCCESS"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TASK272 week9 steady continuation run")
    p.add_argument("--state-latest-json", default="data/phase1_seed10/logs/exhibitions_text_monitored_state_latest.json")
    p.add_argument("--ready-input-csv", default="data/phase1_seed10/logs/exhibitions_text_week8_weekly_run_steady_continuation_ready_input_task271.csv")
    p.add_argument("--escalate-input-csv", default="data/phase1_seed10/logs/exhibitions_text_week8_weekly_run_steady_continuation_escalate_input_task271.csv")
    ...
    p.add_argument("--task271-summary-json", default="data/phase1_seed10/logs/exhibitions_text_week8_weekly_run_steady_continuation_summary_task271.json")
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("task272_%Y%m%dT%H%M%SZ")
    ...
    summary271 = read_json(Path(args.task271_summary_json), default={})

    expected_prior_run_id = str(summary271.get("run_id") or "").strip()
    summary271_ready = str(summary271.get("go_hold_decision") or "").strip() == "CONTROLLED_OPERATION_WEEK8_SUCCESS"
    ...
    add_check("TASK271_WEEK8_SUCCESS", summary271_ready, True)
    ...
    common271 = dict(dict(summary271.get("post_run_verification") or {}).get("common") or {})
    coverage_review_count = int(common271.get("coverage_review_count", 0) or 0)
    reject_candidate_count = int(common271.get("reject_candidate_count", 0) or 0)
    join_blocker_count = int(common271.get("join_blocker_count", 0) or 0)
    ...
    if blocker_fail_count == 0:
        ...
        backup_root = Path(args.trash_root) / f"{timestamp}_pre_task272_week9_steady_{run_id}"
    ...
    if blocker_fail_count == 0 and backup_created and backup_hash_match:
        ...
        out["last_ingestion"] = {
            "task_run_id": run_id,
            "ingestion_mode": "controlled_operation_week9_steady_continuation",
    ...
    ready_summary = {
        "artifact": "exhibitions_text_week9_weekly_run_steady_continuation_ready_summary_task272",
        "task": "TASK272",
        "run_id": run_id,
    ...
    escalate_summary = {
        "artifact": "exhibitions_text_week9_weekly_run_steady_continuation_escalate_summary_task272",
        "task": "TASK272",
        "run_id": run_id,
        "created_at": utc_now_iso(),
    ...
    post = {
        "artifact": "exhibitions_text_week9_weekly_run_steady_continuation_post_run_verification_task272",
        "task": "TASK272",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "monitored_ready": {"monitoring_required_count": monitored_ready_count, "evaluated_count": len(monitored_ready_eval_rows), "policy_breach_count": mr_lane_hold, "evaluation_evidence_file": "exhibitions_text_week9_weekly_run_steady_continuation_monitored_ready_eval_task272.csv"},
    ...
    summary = {
        "artifact": "exhibitions_text_week9_weekly_run_steady_continuation_summary_task272",
        "task": "TASK272",
        "run_id": run_id,
        "created_at": utc_now_iso(),
        ...
        "go_hold_decision": decision,
        "blocker_labels": blocker_labels,
        "next_task_recommendation": {"id": "TASK273", "title": "EXHIBITIONS-TEXT-WEEK10-WEEKLY-RUN-STEADY-CONTINUATION", "ja": "Run Week10 steady weekly sync with reflected lanes and monitored-ready tracking continuity"},
    }

    base = output_dir
    ready_input_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_ready_input_task272.csv"
    ready_summary_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_ready_summary_task272.json"
    ready_manifest_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_ready_manifest_task272.json"
    esc_input_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_escalate_input_task272.csv"
    esc_summary_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_escalate_summary_task272.json"
    esc_manifest_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_escalate_manifest_task272.json"
    preflight_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_preflight_task272.csv"
    backup_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_backup_log_task272.json"
    diff_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_monitored_diff_task272.csv"
    mr_eval_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_monitored_ready_eval_task272.csv"
    by_fair_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_by_fair_task272.csv"
    by_gallery_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_by_gallery_task272.csv"
    post_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_post_run_verification_task272.json"
    stage_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_runbook_stage_result_task272.csv"
    summary_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_summary_task272.json"
    manifest_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_manifest_task272.json"
    report_out = base / "exhibitions_text_week9_weekly_run_steady_continuation_task272.md"
    ...
    print(f"[task272] run_id={run_id} ready={len(ready_rows)} escalate={len(escalate_rows)} holding_excluded={len(holding_rows)} monitored_ready={monitored_ready_count} monitored_hold={mr_lane_hold} boundary={boundary_breach_count} gap={temporal_gap_count} escalate_blocker={escalate_blocker_count} restore={restore_executed} decision={decision}")
    return 0
